...
RSpec.describe "leads_imports/new", type: :view do
  before(:each) do
    assign(:leads_import, LeadsImport.new(
      :title => "MyString",
      :name => "MyString",
      :first_name => "MyString",
      :language => "MyString",
  ...
  it "renders new leads_import form" do
    render

    assert_select "form[action=?][method=?]", leads_imports_path, "post" do
